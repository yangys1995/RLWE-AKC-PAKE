...
void kccon(poly_small *v, const poly *sigma, unsigned char *key){
    ...
    for(i=0;i<PARAM_N/8;i++){
      for(j=0;j<8;j++){
        ...
        uint16_t sigma_a=(sigma->v[8*i+j]<<1);
        uint16_t v_;
        if(sigma_a<BETA){
            key[i]=key[i]<<1;
            v_=sigma_a;
...
void kcrec(unsigned char *key, const poly *sigma, const poly_small *v){
    ...
    for(i=0;i<PARAM_N/8;i++){
      for(j=0;j<8;j++){
            float r=(sigma->v[8*i+j]<<1)/(float)BETA-(v->v[8*i+j]+0.5)/(float)PARAM_G;
            if((r>=-1.5&&r<-0.5)||(r>=0.5&&r<1.5))
              key[i]=(key[i]<<1)|0x1;
            else
              key[i]=key[i]<<1;
...
void kcrec(unsigned char *key, const poly *sigma, const poly_small *v){
    ...
    for(i=0;i<PARAM_N/8;i++){
      for(j=0;j<8;j++){
            double r=(sigma->coeffs[8*i+j]<<1)/(double)BETA-(v->coeffs[8*i+j]+0.5)/(double)PARAM_G;
            int16_t k_=round(r);
            uint16_t k_t=k_+(((k_>>15)&1)<<1);
            key[i]=(key[i]<<1)|(k_t&0x1);
          }
    }
}
*/
...
void akccon(poly_small *v, const poly *sigma, const unsigned char *key,const int *pre){
  int i;
  for(i=0;i<PARAM_N;i++){
    uint16_t v_=(((uint32_t)sigma->v[i]<<6)+pre[i])/(float)PARAM_Q+0.5;
    v->v[i]=v_&0x3f;
  }
}
...
void akcrec(unsigned char *key, const poly *sigma, const poly_small *v){
    ...
    for(i=0;i<PARAM_N/8;i++){
      for(j=0;j<8;j++)
      {
        float r=v->coeffs[i*8+j]/(float)PARAM_G-sigma->coeffs[i*8+j]/(float)PARAM_Q;
        if((r>=-0.75&&r<-0.25)||(r>=0.25&&r<0.75))
            key[i]=(key[i]<<1)|0x1;
        else
            key[i]=key[i]<<1;}
...
void akcrec(unsigned char *key, const poly *sigma, const poly_small *v){
    ...
    for(i=0;i<PARAM_N*BITS_M/8;i++){
      ...
      {
        float r=(v->v[i*8+j]/(float)PARAM_G-sigma->v[i*8+j]/(float)PARAM_Q)*PARAM_M;
...
void akcrec(unsigned char *key, const poly *sigma, const poly_small *v){
    ...
    for(i=0;i<PARAM_N/8;i++){
      ...
      for(j=0;j<8;j++)
      {
        float r=v->coeffs[index+j]/(float)PARAM_G-sigma->coeffs[index+j]/(float)PARAM_Q;
        if(r<0) r+=1;
        if((r<0.25)||(r>=0.75))
            key[i]=(key[i]<<1);
        else
