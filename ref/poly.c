...
void poly_small_frombytes(poly_small *r, const unsigned char *a)
{
  ...
  for(i=0;i<PARAM_N/4;i++)
  {
    ...
    r->coeffs[4*i+1] = (a[3*i+0] >> 6) | ((a[3*i+1] &0x0f)<<2) ;
    r->coeffs[4*i+2] = (a[3*i+1] >> 4) | ((a[3*i+2]&0x03 )<< 4);
...
void poly_uniform(poly *a, const unsigned char *seed)
{
  ...
  shake128_absorb(state, seed, PAK_SEEDBYTES);
...
void poly_pointwise(poly *r, const poly *a, const poly *b)
{
  ...
  for(i=0;i<PARAM_N;i++)
  {
    t       = montgomery_reduce(3186*b->coeffs[i]); /* t is now in Montgomery domain 3186=(2^18)^2 mod 12289 */ 
...
void poly_add(poly *r, const poly *a, const poly *b)
{
  ...
  for(i=0;i<PARAM_N;i++){
    ...
    m = r->coeffs[i] - PARAM_Q;
    c = m;
    c >>= 15;
    r->coeffs[i] = m ^ ((r->coeffs[i]^m)&c);
  }
}
